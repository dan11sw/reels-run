 ...
 */
FileStreamRotator.getStream = function (options, writeHeaderCallback) {
    ...
    if(!fileCount && !fs.existsSync(logfile) && writeHeaderCallback) {
        writeHeaderCallback(logfile);
    }
    ...
    if ((curDate && frequencyMetaData && (staticFrequency.indexOf(frequencyMetaData.type) > -1)) || fileSize > 0) {
        ...
        stream.write = (function (str, encoding) {
            ...
            if (newDate != curDate || (fileSize && curSize > fileSize)) {
                ...
                if(!fs.existsSync(newLogfile) && writeHeaderCallback) {
                    writeHeaderCallback(newLogfile);
                }
