package com.afc.reels_run.network.handlers

import android.util.Log
import com.afc.reels_run.constants.network.main.MainNetworkConstants
...
class SCSocketHandler private constructor() {
    ...
    fun setSocket(cb: (() -> Unit)? = null) {
        ...
        try {
            Log.w(SCSocketHandler.LOG_TITLE, "Попытка подключиться к сокету по адресу: ${MainNetworkConstants.MAIN_API}")
            ...
            mSocket?.on(SocketHandlerConstants.CONNECT) {
                Log.w(SCSocketHandler.LOG_TITLE, "Event \"${SocketHandlerConstants.CONNECT}\": ${mSocket?.connected()}")
            ...
            mSocket?.on(SocketHandlerConstants.DISCONNECT) {
                Log.w(SCSocketHandler.LOG_TITLE, "Event \"${SocketHandlerConstants.DISCONNECT}\": ${mSocket?.connected()}")

                disconnection()
            }
        } catch (e: Exception) {
            Log.e(SCSocketHandler.LOG_TITLE, e.message.toString())
        }
    }
